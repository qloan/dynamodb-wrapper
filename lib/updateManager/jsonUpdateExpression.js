...
class JsonUpdateExpression extends BaseUpdateExpression {
    ...
    performGet(field, defaultValue) {
        field = field || [];
        let output = util.getObjKey(this.json, util.clone(field), defaultValue);
    ...
    performRemove(field) {
        var parentReference = util.getReferenceToParent(this.json, util.clone(field), __FIELD_NOT_FOUND);
        if (parentReference !== __FIELD_NOT_FOUND) {
            var back = util.getLastField(util.clone(field));
