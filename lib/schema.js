...
class Schema extends Observable {
    ...
    constructor(args={createTable: true}) {
        ...
        this.schema = joi.decorateSchema(joi.object().keys(this.schema));
    }
    ...
    create(itemJson, createParams, cb) {
        ...
        async.series([
            (next) => {
                const result = this.schema.validate(createParams.Item);
                if(result.error) {
    ...
    get(key, getParams, cb) {
        ...
        async.series([
            ...
            (next) => {
                const result = this.schema.validate(_.get(data, 'Item'), {abortEarly: false});
                encryptedFields = result.encryptedFields;
                compressedFields = result.compressedFields;
                next();
            },
    ...
    update(itemJson, updateParams, cb) {
        ...
        async.series([
            (next) => {
                const result = this.schema.validate(itemJson);
                if(result.error) {
                    return next(result.error);
                }
                next();
            ...
            (next) => {
                Object.keys(updateParams.ExpressionAttributeValues).forEach((token) => {
                    ...
                    if(typeof(token) == 'object' && (token.action == 'SET' || token.action == 'APPEND')) {
                        ...
                        const result = this.schema.validate(token.obj, {abortEarly: false});
        ...
        async.series([
            ...
            (next) => {
                async.eachSeries(data.Items, (item, eachCb) => {
                    ...
                    let result = this.schema.validate(item, {abortEarly: false});
                    async.series([
                        (seriesNext) => {
