...
class Schema extends Observable {
    ...
    constructor(args={createTable: true}) {
        ...
        if(this.timestamps) {
            ...
            this.schema.updateTimestampHistory = joi.string().isoDate().optional();
        }
    ...
    get(key, getParams, cb) {
        ...
        async.series([
            ...
            (next) => {
                const result = joi.validate(util.getObjKey(data, 'Item'), this.schema, {abortEarly: false});
                encryptedFields = result.encryptedFields;
                compressedFields = result.compressedFields;
                next();
            },
            (next) => {
                ...
                if(typeof(util.getObjKey(data, 'Item')) == 'object' && compressedFields.length) {
                    this.emit('decompress', util.getObjKey(data, 'Item'), compressedFields, next);
                }else {
                    next();
                }
            },
            (next) => {
                ...
                if(typeof(util.getObjKey(data, 'Item')) == 'object' && encryptedFields.length) {
                    this.emit('decrypt', util.getObjKey(data, 'Item'), encryptedFields, next);
                }else {
                    next();
                }
            }
            ...
            (next) => {
                Object.keys(updateParams.ExpressionAttributeValues).forEach((token) => {
                    ...
                    if(typeof(token) == 'object' && (token.action == 'SET' || token.action == 'APPEND')) {
                        token.obj = {};
                        util.setObjKey(token.obj, token.field, token.value);
