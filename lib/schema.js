...
class Schema extends Observable {
    ...
    constructor(args = { createTable: true }) {
        ...
        if (this.timestamps) {
            this.schema.createdAt = joi.string().isoDate().raw();
            this.schema.updatedAt = joi.string().isoDate().raw();
        }

        this.schema.internal_locks_replicate_to_rds = joi.string().isoDate().raw().optional();
    ...
    create(itemJson, createParams, cb) {
        ...
        async.series([
            ...
            (next) => {
                if (!this.s3RecordParams) {
                    return next();
                }
                this.recordInS3({
                    itemJson: createParams.Item,
                    operation: "create"
                }, next);
            },
            (next) => {
    ...
    get(key, getParams, cb) {
        ...
        async.series([
            ...
            (next) => {
                /*
                    note: NOTE_IGNORE_ERROR_ENCRYPT_COMPRESSED
                    It looks like joi.validate will ALWAYS throw an error here if you have encrypted or compressed fields
                    You might say amSchedule = joi.array().compress()
                    And then at THIS point in time, amSchedule will be a buffer, so it will fail validation
                    But result.encryptedFields and result.compressedFields should be populated anyway
                */
    ...
    recordInS3({ itemJson, operation }, callback) {
        let Body;
        try {
            Body = JSON.stringify(itemJson);
        } catch (e) {
            // We can't have catastrophic failure because of this operation. It's not as important as saving data. Fail gracefully.
            console.log(e);
            return callback();
        }
        this.s3.putObject({
            Body,
            Bucket: this.s3RecordParams.Bucket,
            Key: this.getS3Key({ itemJson, operation }),
    ...
    getS3Key({ itemJson, operation }) {
        const key = [];
        key.push(this.tableName);
        key.push(`${this.key.hash}:${itemJson[this.key.hash]}`);
        if (this.key.range) {
            key.push(`${this.key.range}:${itemJson[this.key.range]}`);
        }
        key.push(`${operation}-${util.extendedTimestamp()}`);
        return key.join('/');
    }

    update(itemJson, updateParams, cb) {
        let key = this.getItemKey(itemJson);
        let encryptedFields = [];
        let compressedFields = [];
            ...
            (next) => {
                Object.keys(updateParams.ExpressionAttributeValues).forEach((token) => {
                    ...
                    if (typeof (token) == 'object' && (token.action == 'SET' || token.action == 'APPEND')) {
                        token.obj = {};
                        _.setWith(token.obj, token.field, token.value, Object);
